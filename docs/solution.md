...
│  │ PCM 16kHz │  │ JPEG 640px │  │ MP3 解码  │  │ 时延链路   │  │
│  └─────┬─────┘  └─────┬─────┘  └─────▲────┘  └─────▲─────┘  │
│        │              │              │              │         │
│        └──────────────┴──────────────┴──────────────┘         │
│                            │ WebSocket (JSON)                 │
...
- `asrMs = t1 - t0` — 真实 ASR 尾延迟
- `agentMs = tFirstSentenceAgent - t1` — 大模型首句生成时间
- `ttsMs = tFirstSentenceTts - tFirstSentenceAgent` — TTS 首句合成时间
- `ttfrMs = tFirstSentenceTts - t0` — 首响时延（Time To First Response）
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 算法 | **正则表达式匹配** | 30+ 条模式，优先级顺序匹配 |
| 优先级 | 1. 飞书 → 2. 拍照 → 3. 搜索 → 4. 闲聊 | |
| 飞书模式数 | 22 条 | 中文 + 英文 |
| 拍照模式数 | 30 条 | 中文 + 英文，含手持/眼前/观察/辨认 |
| 搜索模式数 | 22 条 | |
...
| 回复格式 | DEFAULT_REPLY_FORMAT + 角色提示词 | 同左 | 同左 |
...
#### TTS 播报文本提取（speech_summary）

```
1. 去除 URL
2. 按句子分割符拆分
3. 取前 1-2 句，不超过 200 字
4. 如有 URL：追加"答复中有链接，详细内容可以点击查看"
5. 如原文 > 200 字：追加"详细内容可以查看手机页面详情"
```
...
| 指标 | 计算方式 | 含义 |
|------|----------|------|
| **ASR 尾延迟** | `t1 - t0` | 最后音频到达 → ASR 产出文本 |
| **Agent 首句时间** | `tFirstSentenceAgent - t1` | 文本到达 → LLM 产出第一句 |
| **TTS 首句时间** | `tFirstSentenceTts - tFirstSentenceAgent` | LLM 首句 → TTS 输出首句音频 |
| **TTFR（首响时延）** | `tFirstSentenceTts - t0` | 音频到达 → 用户听到第一句话 |
| **总计耗时** | `tLastEvent - t0` | 完整交互链路总时长 |
...
**优化方案：** 将收集和发送合并为单循环，每收到一个完整句子就立即 TTS 合成并发送：

```javascript
for await (const sentence of callVeadkStream(payload)) {
  if (!sentence.speechText) continue;

  sentenceCount++;
  if (sentenceCount === 1) tFirstSentenceAgent = Date.now();

  const tts = await synthesizeSpeech(sentence.speechText);
  if (sentenceCount === 1) tFirstSentenceTts = Date.now();

  send(ws, { ...sentence, audioBase64: tts.audioBase64, timing: ... });
}
```

**优化效果：** TTFR 减少约 **50%**，用户听到第一句话的延迟从 "LLM 全长 + TTS 全长" 缩短为 "LLM 首句 + TTS 首句"。
...
**效果：** `asrMs = t1 - t0` 精确反映火山引擎 ASR 的真实尾延迟（通常 200-1500ms）。

### 4.4 拍照意图增强

**问题：** 原拍照意图仅匹配"拍照"、"这是什么"、"眼前"等少数关键词，"我手上拿着的什么"、"帮我看看这个"等自然表达无法触发拍照。

**优化方案：** 将 `PHOTO_PATTERNS` 从 14 条扩展到 30 条，覆盖三类场景：

| 类别 | 新增模式 | 匹配示例 |
|------|----------|----------|
| 手持/眼前 | `/手上.*什么/`, `/拿着.*什么/`, `/手里.*什么/`, `/面前.*什么/`, `/桌上.*什么/`, `/看见.*什么/`, `/那是什么/` | "我手上拿着的什么" |
| 观察/辨认 | `/帮我看看/`, `/看一下/`, `/看看这/`, `/瞧瞧/`, `/认得/`, `/认识/` | "帮我看看这个" |
...
**优化方案：** 在 ArkClaw 任务异步执行的同时，同步返回一条确认语音：
...
// 同步返回 comfort ack
const comfortTts = await synthesizeSpeech("收到，正在通过飞书帮你处理，完成后告诉你。");
send(ws, { type: ASSISTANT_RESULT, phase: "ack", ... });
```

**效果：** 用户立即听到确认语音（~1s），之后异步收到飞书任务结果，体验接近即时反馈。
...
**优化方案：**
1. LLM 首句到达时立即创建 `_streaming` 状态对象
2. 后续句子追加到同一对象并刷新 UI
3. 拍照任务创建 placeholder "📷 正在分析拍摄的照片…"
4. 首句到达时替换 placeholder（`phDedup` 去重逻辑）
...
| WebSocket 断开 | ws.on("close") 清理 session |
...
│   │   └── package.json     # express + ws + tweetnacl + @volcengine/arkclaw
